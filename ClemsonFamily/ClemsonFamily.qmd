...
housing <- ClemsonFamilyData %>% filter(CampusHousing == TRUE)
...
edges_housing <- create_edges(housing, "CampusHousing")
...
nodes <- ClemsonFamilyData %>%
  ...
  select(ID, CampusHousing, WoodlandBurial) %>%
  mutate(label = ID)
...
ggraph(worked_and_died_graph, layout = "fr") +
  ...
  geom_node_point(aes(color = case_when(
    CampusHousing == TRUE & WoodlandBurial == TRUE ~ "Both",
    CampusHousing == TRUE ~ "Housing Only",
    WoodlandBurial == TRUE ~ "Burial Only"
  )), size = 3) +
  geom_node_text(aes(label = label), size = 2, repel = TRUE) +
  scale_edge_color_manual(values = c("CampusHousing" = "blue", "WoodlandBurial" = "green")) +
