...
#load .csv files
nodes <- read.csv("ClemsonFamilyNodesListVer3.csv", stringsAsFactors = FALSE)
nodes <- as.data.frame(nodes)

edges <- read.csv("CFEdgesListVer2.csv", stringsAsFactors = FALSE) 
...
```{r, echo=FALSE}
#Building a network of church membership and clemson department connectedness

# Assume your dataset is already loaded into a data frame called `df`
df <- read.csv("ClemsonFamily_files/files/CFEdgesList.csv", stringsAsFactors = FALSE)
...
# 2. Create edge list: connect people who share the same Group
edges <- df_long %>%
  edges <- df_long %>%
  group_by(Group) %>%
  filter(n() > 1) %>% # Exclude groups with fewer than 2 members
  reframe(pairs = combn(ID, 2, simplify = FALSE)) %>% # Use reframe instead of summarise
  unnest(pairs) %>%
  mutate(
    from = map_chr(pairs, 1), # Extract the first element of each pair
    to = map_chr(pairs, 2)    # Extract the second element of each pair
