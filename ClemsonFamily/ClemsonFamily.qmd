...
connection_edges <- connections_long %>%
...
# 4. Create node list
connection_nodes <- ClemsonFamilyData %>%
  filter(ID %in% unique(c(connection_edges$from, connections_edges$to))) %>%
  select(ID, ClemsonDepartment) %>%
  rename(label = ID)

# 5. Build graph
connections_network <- graph_from_data_frame(d = connection_edges, vertices = connection_nodes, directed = FALSE)
